...
import { getSiwsSessionSigs } from "./litSiws";
...
function App() {
  ...
  const handleSignIn = async (siws: SiwsObject) => {
    ...
    setIsPending(true);
    try {
      const sessionSigs = await getSiwsSessionSigs(siws);
      if (sessionSigs) {
        setAuthenticatedAddress(siws.siwsInput.address);
        setAuthError(null);
        console.log("Session Sigs generated successfully", sessionSigs);
      } else {
        setAuthError("Failed to generate Session Sigs");
...
const MainContent: React.FC<MainContentProps> = ({
  ...
  return (
    <>
      ...
      {authenticatedAddress && (
        <div className="card">
          <h3>Session Sigs Generated</h3>
          <p>Session Sigs generated successfully</p>
          <hr />
        </div>
      )}
    </>
