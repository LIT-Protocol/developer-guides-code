...
//         console.log("OpenAI Response:", JSON.stringify(resultInf, null, 2));
...
(async () => {
    ...
    async () => {
      ...
      let openAiResponse = await fetch(
        "https://api.openai.com/v1/chat/completions",
        {
          ...
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${apiKey}`, // in jsParams
          },
          body: JSON.stringify({
            model: "gpt-4o-mini",
            messages: messages,
  ...
  await LitActions.signEcdsa({
    ...
    publicKey, // in jsParams
    sigName: "signedOpenAiResponse",
  });

  LitActions.setResponse({ response: `Signed message: ${toSign}` });
})();
