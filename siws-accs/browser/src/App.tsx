...
import { litSiws } from "./litSiws";
import { getSolRpcConditions } from "./litSiws";
...
function App() {
  ...
  const handleSignIn = async (siws: SiwsObject) => {
    ...
    setIsPending(true);
    try {
      const solRpcConditions = await getSolRpcConditions(
        siws.siwsInput.address
      );
      const isAuthenticated = await litSiws(siws, solRpcConditions);
      if (isAuthenticated) {
        setAuthenticatedAddress(siws.siwsInput.address);
        setAuthError(null);
      } else {
        setAuthError("SIWS message failed authentication");
