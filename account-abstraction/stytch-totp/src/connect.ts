...
const stytchSmsAuth = async () => {
  ...
  await client.session.authenticate({session_duration_minutes: 60})

  console.log("🔄 Requesting SMS OTP from Stytch...");
  const stytchResponse = await client.totps.create({ expiration_minutes: 10 });
  console.log("✅ Sent SMS OTP requestto Stytch");
